		...
		function controllerFunc(scope, el) {
			...
			function lazyLoadTheComponentModule(newBrand, forceReload) {
				...
				if (!$ocLazyLoad.isLoaded(moduleName)) {
					$ocLazyLoad.load({
						name: moduleName,
						files: ['app/build/' + $rootScope.brand + '/css/dmc.app.css', 'app/build/' + moduleName + '.js'],
						cache: !forceReload
					...
					$ocLazyLoad.load({
						name: 'app',
						files: ['app/build/' + newBrand + '/css/dmc.app.css', 'app/build/dmc.app.js'],
						cache: !forceReload
					});
					vm.getComponentPage();
				}
			...
			function unloadCSSModules (oldBrand) {
				$('link[href="app/build/' + oldBrand + '/css/dmc.app.css"]').remove();
			}
